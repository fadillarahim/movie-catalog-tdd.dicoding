...
class FavoriteMovieSearchPresenter {
  ...
  async _searchMovies(latestQuery) {
    ...
    if (this.latestQuery.length > 0) {
      foundMovies = await this._favoriteMovies._searchMovies(this.latestQuery);
    } else {
      foundMovies = await this.favoriteMovies.getAllMovies();
    }
  ...
  _showFoundMovies(movies) {
    const html = movies.reduce((carry, movie) => carry.concat(`
    <li class="movie">
